...
# For example, they now know that the North Pole decorations need to be finished
...
# an L or R which indicates whether the rotation should be to the left (toward lower numbers)
# or to the right (toward higher numbers). Then, the rotation has a distance
...
class BreakLocker:
    ...
    def calculate_left(self, x: int) -> int:
        self.position = (self.position - x) % 100
        return self.position

    def calculate_right(self, x: int) -> int:
        self.position = (self.position + x) % 100
        return self.position
